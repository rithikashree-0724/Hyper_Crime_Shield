        ...
        <section className="flex flex-col gap-6 mb-10" id="response">
            ...
            <div className="grid gap-4">
                {[
                    ...
                    <div key={idx} className="flex items-start gap-4 p-5 rounded-xl bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-[#3b4754]">
                        <div className={`${item.active ? 'bg-primary text-white' : 'bg-surface-light dark:bg-slate-700 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600'} rounded-lg size-10 flex items-center justify-center shrink-0 font-bold text-lg shadow-lg`}>
                            {item.step}
                        </div>
